# Github Actions ImmortalWrt 360T7

- Openwrt源码是[padavanonly/immortalwrtARM](https://github.com/padavanonly/immortalwrtARM/tree/mt7981)。
...
```
进入目录`\\wsl$\Ubuntu*\home\*\immortalwrtARM`复制一下这个`seed.config`的文本内容到项目根目录的`360T7.config`文件中，这样就不用每次都SSH连接到 Actions生成编译配置，真正一键编译。
